...
module.bundle.HMR_BUNDLE_ID = "173d284b79e4eaeb";
"use strict";
...
},{}],"ghJDj":[function(require,module,exports) {
...
function initApp() {
    const stepHandler = (e)=>{
        ...
        if (e.target.dataset.steptype === "prev") return aggiornaForm(config.STEP_PREV);
        //Se siamo all'ultimo step inviamo il form
        if (config.currentStep === config.STEPS) inviaForm();
        //Controllo correttezza campi
        if (validaCampi()) aggiornaForm(config.STEP_NEXT);
    };
...
function validaCampi() {
    ...
    getCampi().forEach((formElem)=>{
        const fieldValue = formElem.value, checks = [
            formElem.type === "email" && !(0, _validatorDefault.default).isEmail(fieldValue),
            formElem.required && (0, _validatorDefault.default).isEmpty(fieldValue)
        ];
        ...
        if (formElem.type === "password" && formElem.id === "password") result = checkPassword(formElem, fieldValue);
    });
    return result;
}
function checkPassword(formElem, fieldValue) {
    const confirm = document.querySelector("#password-confirm").value;
    if (!(0, _validatorDefault.default).isStrongPassword(fieldValue) || !(0, _validatorDefault.default).equals(fieldValue, confirm)) {
        reimpostaClassi(formElem.parentElement, "has-error");
        return false;
    }
    reimpostaClassi(formElem.parentElement, "", "has-error");
...
function getCampi() {
    const idStepWrapper = "step-" + config.currentStep, selettore = `#${idStepWrapper} :is(input, textarea)`, formFields = document.querySelectorAll(selettore);
    return formFields;
}
...
function aggiornaIndicatori() {
    config.indicator.forEach((elem, index)=>{
        if (index < config.currentStep - 1) reimpostaClassi(elem, "success", "next", "current");
        else if (index === config.currentStep - 1) reimpostaClassi(elem, "current", "success", "next");
        else reimpostaClassi(elem, "next", "success", "current");
    });
}
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","validator":"bliRI"}],"gkKU3":[function(require,module,exports) {
